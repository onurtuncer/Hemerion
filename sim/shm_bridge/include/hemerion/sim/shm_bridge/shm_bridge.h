...
/// @file shm_bridge.h
/// @brief Lockstep step handshake over a shared-memory segment.
///
...
{
  kReady,     ///< The peer posted; the out-parameters are filled in.
  kShutdown,  ///< kShutdownRequested was observed; the run is over.
  kTimedOut,  ///< The timeout elapsed with the phase unchanged.
};

/// @brief Lockstep step handshake between the FMI master and its peer over a
/// shared-memory segment.
///
/// One instance per process per run: the master gets one from
/// create_master(), the peer from open_peer(). The master-side and peer-side
/// members below are not interchangeable -- calling the wrong side's members
/// stalls the handshake rather than failing loudly.
class ShmBridge
{
public:
