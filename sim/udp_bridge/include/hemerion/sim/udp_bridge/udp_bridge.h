...
/// @file udp_bridge.h
/// @brief Lockstep step handshake over a UDP datagram pair.
///
...
{
  kReady,     ///< A datagram arrived; the out-parameters are filled in.
  kShutdown,  ///< A kShutdown packet arrived; the run is over.
  kTimedOut,  ///< The timeout elapsed with no datagram.
};

/// @brief Lockstep step handshake between the FMI master and its peer over a
/// UDP datagram pair.
///
/// The datagram-transport counterpart to ShmBridge, for a peer on another
/// host. One instance per process per run; the master-side and peer-side
/// members below are not interchangeable.
class UdpBridge
{
public:
