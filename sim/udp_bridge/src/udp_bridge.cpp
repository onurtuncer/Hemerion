...
std::optional<UdpBridge> UdpBridge::create_master(const std::string& local_address, std::uint16_t local_port,
...
std::optional<UdpBridge> UdpBridge::create_peer(const std::string& local_address, std::uint16_t local_port,
                                                  const std::string& peer_address, std::uint16_t peer_port) {
  std::optional<UdpSocket> socket = UdpSocket::create(local_address, local_port, peer_address, peer_port);
  if (!socket) {
    return std::nullopt;
...
WaitResult UdpBridge::wait_for_inputs(ChannelFrame& inputs, double& sim_time_s, double& dt_s,
