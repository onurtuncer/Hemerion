...
/// @file i2c_shm_protocol.h
/// @brief Shared-memory wire format of a simulated I2C bus.
///
...
/// Layout version stamped into I2cBusRegion::protocol_version. Bump it on any
...
struct I2cBusRegion
{
  ...
  std::atomic<std::uint64_t> transaction_index{ 0 };  ///< Transactions completed so far this run.
  ...
  std::array<std::uint8_t, kMaxPhaseBytes> write_bytes{};  ///< Controller -> peripheral; `write_length` valid.
  std::array<std::uint8_t, kMaxPhaseBytes> read_bytes{};   ///< Peripheral -> controller; `read_length` valid.
};
