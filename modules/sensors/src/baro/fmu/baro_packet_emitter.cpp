...
/// @file baro_packet_emitter.cpp
/// @brief Implements the Hemerion barometer frame encoder declared in
/// baro_packet_emitter.h. Byte offsets mirror
/// BaroPacketParser::decode_raw_sample() exactly.
...
void write_u32(BaroPacketEmitter::Frame& frame, std::size_t offset, std::uint32_t value)
{
...
void write_i32(BaroPacketEmitter::Frame& frame, std::size_t offset, std::int32_t value)
{
  write_u32(frame, offset, static_cast<std::uint32_t>(value));
}

void write_u64(BaroPacketEmitter::Frame& frame, std::size_t offset, std::uint64_t value)
{
...
BaroPacketEmitter::Frame BaroPacketEmitter::encode_raw_sample(const BaroRawSample& raw)
{
  Frame frame{};
  frame[0] = kBaroPacketSync1;
  frame[1] = kBaroPacketSync2;
  frame[2] = kBaroPacketRawSampleId;
  frame[3] = static_cast<std::uint8_t>(kBaroPacketRawSamplePayloadLength & 0xFFU);
  frame[4] = static_cast<std::uint8_t>((kBaroPacketRawSamplePayloadLength >> 8) & 0xFFU);

  constexpr std::size_t kPayload = 5;  // first payload byte, after sync/id/length
  write_i32(frame, kPayload + 0, raw.pressure);
  write_i32(frame, kPayload + 4, raw.temperature);
