...
/// @file fmu_main.cpp
/// @brief FMI 2.0 Co-Simulation slave entry points for the radar altimeter
/// hardware simulator.
///
/// This FMU has no FMI output variables -- its effect is a UDP side
/// channel: each fmi2DoStep() call turns the current truth
/// height-above-ground input into noisy raw range register counts
/// (RadAltNoiseModel, including loss-of-track beyond max range), encodes
/// them as Hemerion radar altimeter raw-sample frames (RadAltPacketEmitter),
/// and sends them to a fixed UDP peer (UdpSender), so the real, unmodified
/// RadAltPacketParser + convert_raw_to_si() on the firmware side decodes
/// them exactly as it would bytes from a physical part.
///
/// Like the IMU FMU (Hemerion/imu/fmu/fmu_main.cpp, whose structure this
/// file mirrors), the `sample_rate_hz` parameter (default 25 Hz) makes each
...
/// HEMERION_RADALT_FMU_UDP_HOST / HEMERION_RADALT_FMU_UDP_PORT (defaults
/// 127.0.0.1:5765) rather than exposed as an FMI string parameter -- this
...
constexpr fmi2ValueReference kVrHeightAgl = 0;
constexpr fmi2ValueReference kVrSampleRateHz = 1;

// Must match model_description.xml's <fmiModelDescription guid="...">.
constexpr char kExpectedGuid[] = "{6F8252FE-FD6E-48CD-9C51-8F9EDB00765A}";
...
struct RadAltFmuInstance
{
  RadAltNoiseModel noise_model;
  UdpSender sender;
...
RadAltFmuInstance* to_instance(fmi2Component component) { return static_cast<RadAltFmuInstance*>(component); }
...
#endif
  const char* host_env = std::getenv("HEMERION_RADALT_FMU_UDP_HOST");
  const char* port_env = std::getenv("HEMERION_RADALT_FMU_UDP_PORT");
#if defined(__clang__)
...
#pragma warning(pop)
  ...
  auto* instance = new RadAltFmuInstance{ RadAltNoiseModel{},
                                          std::move(*sender),
                                          RadAltTruthSample{},
...
{
  RadAltFmuInstance* instance = to_instance(component);
  instance->truth = RadAltTruthSample{};
                                            ...
                                            const fmi2Real value[])
{
  ...
  for (std::size_t i = 0; i < nvr; ++i)
  {
    ...
    {
      case kVrHeightAgl:
        instance->truth.height_agl_m = value[i];
        break;
...
HEMERION_FMI2_EXPORT fmi2Status fmi2GetReal(fmi2Component component,
                                            ...
                                            fmi2Real value[])
{
  const RadAltFmuInstance* instance = to_instance(component);
  for (std::size_t i = 0; i < nvr; ++i)
  {
    switch (vr[i])
    {
      case kVrHeightAgl:
        value[i] = instance->truth.height_agl_m;
        break;
      case kVrSampleRateHz:
