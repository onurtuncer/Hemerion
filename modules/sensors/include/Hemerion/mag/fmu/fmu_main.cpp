...
/// @file fmu_main.cpp
/// @brief FMI 2.0 Co-Simulation slave entry points for the magnetometer
/// hardware simulator.
///
...
/// magnetic field inputs into noisy raw register counts (MagNoiseModel),
/// encodes them as Hemerion magnetometer raw-sample frames
/// (MagPacketEmitter), and sends them to a fixed UDP peer (UdpSender), so
/// the real, unmodified MagPacketParser + convert_raw_to_si() on the
...
/// file mirrors), the `sample_rate_hz` parameter (default 100 Hz) makes
/// each fmi2DoStep emit round(step * rate) frames; the truth inputs are
/// zero-order-held across the step, so sub-step samples differ in noise
/// draw and timestamp only.
///
/// UDP destination is read once at fmi2Instantiate() from
/// HEMERION_MAG_FMU_UDP_HOST / HEMERION_MAG_FMU_UDP_PORT (defaults
/// 127.0.0.1:5766) rather than exposed as an FMI string parameter -- this
...
constexpr fmi2ValueReference kVrMagX = 0;
constexpr fmi2ValueReference kVrMagY = 1;
constexpr fmi2ValueReference kVrMagZ = 2;
constexpr fmi2ValueReference kVrSampleRateHz = 3;

// Must match model_description.xml's <fmiModelDescription guid="...">.
constexpr char kExpectedGuid[] = "{23191CA1-1B4E-4D8B-A569-A63C8F908FBA}";
...
struct MagFmuInstance
{
  MagNoiseModel noise_model;
  UdpSender sender;
...
MagFmuInstance* to_instance(fmi2Component component) { return static_cast<MagFmuInstance*>(component); }
...
#endif
  const char* host_env = std::getenv("HEMERION_MAG_FMU_UDP_HOST");
  const char* port_env = std::getenv("HEMERION_MAG_FMU_UDP_PORT");
#if defined(__clang__)
...
#pragma warning(pop)
  ...
  auto* instance = new MagFmuInstance{ MagNoiseModel{},
                                       std::move(*sender),
                                       MagTruthSample{},
...
{
  MagFmuInstance* instance = to_instance(component);
  instance->truth = MagTruthSample{};
                                            ...
                                            const fmi2Real value[])
{
  ...
  for (std::size_t i = 0; i < nvr; ++i)
  {
    ...
    {
      case kVrMagX:
        instance->truth.mag_x_ut = value[i];
        break;
      case kVrMagY:
        instance->truth.mag_y_ut = value[i];
        break;
      case kVrMagZ:
        instance->truth.mag_z_ut = value[i];
        break;
...
HEMERION_FMI2_EXPORT fmi2Status fmi2GetReal(fmi2Component component,
                                            ...
                                            fmi2Real value[])
{
  const MagFmuInstance* instance = to_instance(component);
  for (std::size_t i = 0; i < nvr; ++i)
  {
    switch (vr[i])
    {
      case kVrMagX:
        value[i] = instance->truth.mag_x_ut;
        break;
      case kVrMagY:
        value[i] = instance->truth.mag_y_ut;
        break;
      case kVrMagZ:
        value[i] = instance->truth.mag_z_ut;
        break;
      case kVrSampleRateHz:
